...
static bfd_reloc_code_real_type
elf32_aarch64_bfd_reloc_from_type (unsigned int r_type)
{
...
static reloc_howto_type *
elf32_aarch64_howto_from_type (unsigned int r_type)
{
  ...
  val = elf32_aarch64_bfd_reloc_from_type (r_type);
...
static void
elf32_aarch64_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
		  ...
		  bfd_vma offset, bfd_vma value)
{
  ...
  howto = elf32_aarch64_howto_from_type (r_type);
  place = (input_section->output_section->vma + input_section->output_offset
	   + offset);

  r_type = elf32_aarch64_bfd_reloc_from_type (r_type);
			...
			unsigned long r_symndx)
{
  bfd_reloc_code_real_type bfd_r_type
    = elf32_aarch64_bfd_reloc_from_type (r_type);
			 ...
			 Elf_Internal_Rela *rel, struct elf_link_hash_entry *h)
{
  ...
  switch (elf32_aarch64_bfd_reloc_from_type (r_type))
    {
    case BFD_RELOC_AARCH64_TLSDESC_ADR_PAGE21:
    case BFD_RELOC_AARCH64_TLSGD_ADR_PAGE21:
      if (is_local)
  ...
  for (; rel < relend; rel++)
    {
      ...
      howto = bfd_reloc.howto = elf32_aarch64_howto_from_type (r_type);
      ...
      if (relaxed_bfd_r_type != bfd_r_type)
	{
	  ...
	  r_type = howto->type;
	  r = elf32_aarch64_tls_relax (globals, input_bfd, contents, rel, h);
	  unresolved_reloc = 0;
	}
      else
...
static bfd_boolean
elf32_aarch64_find_nearest_line (bfd *abfd,
				 asymbol **symbols,
				 asection *section,
				 bfd_vma offset,
				 const char **filename_ptr,
...
static void
elf32_aarch64_post_process_headers (bfd *abfd,
				    struct bfd_link_info *link_info)
{
...
static bfd_boolean
elf32_aarch64_allocate_local_dynrelocs (void **slot, void *inf)
{
  struct elf_link_hash_entry *h
  ...
  return elf32_aarch64_allocate_dynrelocs (h, inf);
}
				     ...
				     struct bfd_link_info *info)
{
  ...
  htab_traverse (htab->loc_hash_table,
		 elf32_aarch64_allocate_local_dynrelocs,
		 info);
				       ...
				       struct bfd_link_info *info)
{
  ...
  memcpy (plt_entry, elf32_aarch64_small_plt_entry, PLT_SMALL_ENTRY_SIZE);
				     ...
				     *htab)
{
  ...
  memcpy (htab->root.splt->contents, elf32_aarch64_small_plt0_entry,
	  PLT_ENTRY_SIZE);
  ...
  if (htab->root.splt && htab->root.splt->size > 0)
      ...
      if (htab->tlsdesc_plt)
	{
	  ...
	  memcpy (htab->root.splt->contents + htab->tlsdesc_plt,
		  elf32_aarch64_tlsdesc_small_plt_entry,
		  sizeof (elf32_aarch64_tlsdesc_small_plt_entry));
...
#define bfd_elf32_find_nearest_line		\
  elf32_aarch64_find_nearest_line
...
#define elf_backend_post_process_headers	\
  elf32_aarch64_post_process_headers
