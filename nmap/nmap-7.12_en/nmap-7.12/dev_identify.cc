...
int di_get_telnet_info(unsigned int ipaddr, int port,char *telnet_text, int size)
{
	...
	if (port == 23)
	{
		snprintf(cmd,69,"python /usr/local/devscan/devscan_telnet.py "NMAP_FMT_IP" 2>/dev/null",NMAP_STR_IP(ipaddr));
	}
	else
	{
		return -1;
...
int nmap_di_lib_destory(void)
{
	di_dev_fp_destory();
	di_content_fp_destory();
	di_telnet_fp_destory();
...
int nmap_di_lib_match(unsigned int ipaddr,struct nmap_identify_s *nmap_identify,struct devscan_result_s *nmap_result)
{
	...
	{
		snprintf(nmap_result->devtype_cn_name,NMAP_NAME_LEN-1,"%s",devtype_info[nmap_result->devtype_id].cn_name);
		snprintf(nmap_result->devtype_en_name,NMAP_NAME_LEN-1,"%s",devtype_info[nmap_result->devtype_id].en_name);
...
int devscan_identify(unsigned int ipaddr,struct devscan_result_s *devscan_result)
{
	...
	sprintf(command,"nmap --host-timeout 120 -O "NMAP_FMT_IP"",NMAP_STR_IP(ipaddr));
