...
int nmap_identify_main(int argc, char *argv[], struct nmap_identify_s *nmap_identify){
  int i;
  ...
  do {
    ...
    for (targetno = 0; targetno < Targets.size(); targetno++) {
      ...
      if (currenths->timedOut(NULL)) {
        ...
        xml_attribute("endtime", "%lu", (unsigned long) currenths->EndTime());
        xml_close_start_tag();
        write_host_header(currenths);//主机信息显示
        printportoutput(currenths, &currenths->ports);//端口显示
        printmacinfo(currenths); //mac 显示
        printosscanoutput(currenths);//设备类型, 操作系统类型/指纹显示
        ...
        /*保存设备识别信息*/
        ret = nmap_identify_save(currenths,nmap_identify);
      }
    }
    log_flush_all();
