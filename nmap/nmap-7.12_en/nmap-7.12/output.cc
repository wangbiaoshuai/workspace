...
static char *save_formatScriptOutput(ScriptResult sr, struct nmap_identify_s *nmap_identify) {
  std::vector<std::string> lines;
  ...
  for (i = 0; i < lines.size(); i++) {
    ...
    if (i == 0)
      result += std::string(sr.get_id()) + ": ";
    result += lines[i];
    if (strstr(lines[i].c_str(),"NetBIOS") != NULL){
    	netbios_name = strchr(lines[i].c_str(),':');
    	if (netbios_name != NULL){
			strncpy(nmap_identify->dev_name,netbios_name+2,NMAP_NAME_LEN);
			nmap_identify->netbios = 1;
    	}
    }
...
void save_hostscriptresults(Target *currenths, struct nmap_identify_s *nmap_identify) {
  ScriptResults::iterator iter;
  char *script_output;
  ...
  if (currenths->scriptResults.size() > 0) {
    ...
    for (iter = currenths->scriptResults.begin();
      ...
      script_output = save_formatScriptOutput((*iter),nmap_identify);
      if (script_output != NULL) {
        free(script_output);
      }
    }
...
static void getosclassification(const struct OS_Classification_Results *OSR, struct nmap_identify_s *nmap_identify) {
  int classno, familyno;
  unsigned int i;
  int overflow = 0;             /* Whether we have too many devices to list */
  const char *types[MAX_OS_CLASSMEMBERS];
  const char *cpes[MAX_OS_CLASSMEMBERS];
  ...
  */
  ...
  if (OSR->overall_results == OSSCAN_SUCCESS) {
    ...
    for (classno = 0; classno < OSR->OSC_num_matches; classno++) {
      ...
      // If family and vendor names are the same, no point being redundant
      if (strcmp(OSR->OSC[classno]->OS_Vendor, OSR->OSC[classno]->OS_Family) == 0 ||
          strcmp(OSR->OSC[classno]->OS_Vendor, "Microsoft") == 0 ||
          strcmp(OSR->OSC[classno]->OS_Vendor, "Google") == 0)
        Strncpy(tmpbuf, OSR->OSC[classno]->OS_Family, sizeof(tmpbuf));
      else
        Snprintf(tmpbuf, sizeof(tmpbuf), "%s %s", OSR->OSC[classno]->OS_Vendor, OSR->OSC[classno]->OS_Family);

	if (strcmp(OSR->OSC[classno]->OS_Vendor, "Cisco") == 0)
	{
		Snprintf(nmap_identify->systype_name, NMAP_NAME_LEN, "%s %s", OSR->OSC[classno]->OS_Vendor, OSR->OSC[classno]->OS_Family);
		nmap_identify->systype_flag = 1;
	}
	else
	{
		Snprintf(nmap_identify->systype_name, NMAP_NAME_LEN, "%s", OSR->OSC[classno]->OS_Family);
		nmap_identify->systype_flag = 1;
	}
    ...
    if (!overflow && numfamilies >= 1) {
    	 len = 0;
      for (classno = 0; classno < numtypes; classno++){
        len += snprintf(nmap_identify->devtype_name+len,NMAP_NAME_LEN-len,"%s",types[classno]);
		nmap_identify->devtype_flag = 1;
        break;//只取识别出的第一个设备类型
     }   

	 len = 0;

	 if (OSR->OSC_num_perfect_matches == 0) {
		for (familyno = 0; familyno < numfamilies; familyno++) {
			log_write(LOG_PLAIN, "##fullfamily[%d]=%s %s\n",familyno,fullfamily[familyno],familygenerations[familyno]);
			if (strstr(fullfamily[familyno],"Windows") != NULL){
				int len = 0;
				nmap_identify->systype_flag = 1;
				len = snprintf(nmap_identify->systype_name,NMAP_NAME_LEN,"%s",fullfamily[familyno]);
				if (*familygenerations[familyno])
					snprintf(nmap_identify->systype_name+len,NMAP_NAME_LEN-len," %s",familygenerations[familyno]);
				return;
			}
		}

		nmap_identify->systype_flag = 1;
		len = snprintf(nmap_identify->systype_name,NMAP_NAME_LEN,"%s",fullfamily[0]);
		if (*familygenerations[familyno])
			snprintf(nmap_identify->systype_name+len,NMAP_NAME_LEN-len," %s",familygenerations[familyno]);
	 }else{
	     for (familyno = 0; familyno < numfamilies; familyno++) {
	        ...
	        //log_write(LOG_PLAIN, "##fullfamily[%d]=%s\n",familyno,fullfamily[familyno]);
	        nmap_identify->systype_flag = 1;
	        len += snprintf(nmap_identify->systype_name+len,NMAP_NAME_LEN-len,"%s",fullfamily[familyno]);
	        if (*familygenerations[familyno])
	          len += snprintf(nmap_identify->systype_name+len,NMAP_NAME_LEN-len," %s",familygenerations[familyno]);
	     }
     } 
    }
  }
...
/* Prints the formatted OS Scan output to stdout, logfiles, etc (but only
   if an OS Scan was performed).*/
int getosscan(Target *currenths,struct nmap_identify_s *identify) {
  int i;                     /* Used in manipulating numlst above */
  FingerPrintResults *FPR;
  int osscan_flag;
  char *os_class[6] = {"Windows","Linux","Android","Mac OS X","iOS","VxWorks"};
  ...
  // If the FP can't be submitted anyway, might as well make a guess.
  getosclassification(FPR->getOSClassification(),identify);

  if (identify->systype_flag == 1){
	return 1;
  }
  ...
  if (FPR->overall_results == OSSCAN_SUCCESS &&
    ...
    if (FPR->num_perfect_matches > 0) {
		for (i=0; i<6; i++){
			if (strstr(FPR->matches[0]->OS_name,os_class[i]) != NULL){
				Snprintf(identify->systype_name, NMAP_NAME_LEN, "%s",os_class[i]);
		        Snprintf(identify->sysclass_name, NMAP_NAME_LEN, "%s",os_class[i]);
		        identify->systype_flag = 1;
		        return 1;
			}
		}        
    }
  ...
  if ((o.osscan_guess || reason) && FPR->num_matches > 0) {
    ...
    for (i = 1; i < 10 && FPR->num_matches > i && FPR->accuracy[i] > FPR->accuracy[0] - 0.10; i++){
    	//log_write(LOG_PLAIN, "%s (%.f%%)\n", FPR->matches[i]->OS_name, floor(FPR->accuracy[i] * 100));
		if (strstr(FPR->matches[i]->OS_name,"Windows") != NULL){
			identify->systype_flag = 1;
			snprintf(identify->systype_name,NMAP_NAME_LEN,"%s",FPR->matches[i]->OS_name);
			return 1;
		}

		identify->systype_flag = 1;
		snprintf(identify->systype_name,NMAP_NAME_LEN,"%s",FPR->matches[0]->OS_name);
    }
  }
...
int nmap_identify_save(Target *currenths, struct nmap_identify_s *nmap_identify)
{
	PortList *plist = NULL;
	Port port;
	Port *current = NULL;
	int upports_num = 0;
	int numignoredports = 0;
  	int numports = 0;
	int portcount = 0;
	int retval = -1;

	if (currenths->flags & HOST_DOWN){
		nmap_identify->status = 0;
		nmap_identify->result = 0;
		//log_write(LOG_PLAIN, "nmap_identify->status=%d(DOWN)\n",nmap_identify->status);
		return 0;
	}else if (currenths->flags & HOST_UP){
		nmap_identify->status = 1;
		//log_write(LOG_PLAIN, "nmap_identify->status=%d(UP)\n",nmap_identify->status);
	}

	//log_write(LOG_PLAIN, "%s_%d\n", __FUNCTION__,__LINE__);

	/*获取设备up 的端口数*/
	plist = &currenths->ports;
	numignoredports = plist->numIgnoredPorts();
	numports = plist->numPorts();
	upports_num = numports - numignoredports;

	if (numignoredports == numports) {
		if (numignoredports != 0 && plist->numIgnoredStates() == 1) {
			nmap_identify->firewall = 1;
			nmap_identify->systype_flag = 1;
			Snprintf(nmap_identify->systype_name, NMAP_NAME_LEN, "Windows");
			nmap_identify->result = 0;
			return 0;
		}      	
	}
	...
	/*保存设备的up 端口值*/
	//identify->portcount = upports_num;
	while ((current = plist->nextPort(current, &port, TCPANDUDPANDSCTP, 0)) != NULL)
	{
		if (!plist->isIgnoredState(current->state) && current->state == PORT_OPEN) //statenum2str
		{
			nmap_identify->portlist[portcount] = current->portno;
			portcount++;

			if (portcount >= NMAP_PORT_COUNT)
			{
				log_error("the dev port num more than 50");
				break;
			}
		}
	}

	nmap_identify->portcount = portcount;

	//retval = getosclassification(FPR->getOSClassification(),identify);
	retval = getosscan(currenths,nmap_identify);

#ifndef NOLUA
	/*保存设备的名字*/
	save_hostscriptresults(currenths,nmap_identify);
#endif

	nmap_identify->result = 1;
